...
function ContainerValidator() {
  ...
  this.errorMessages = [];
  this.ownerCode = [];
  this.productGroupCode = undefined;
  ...
  this.isValid = function (containerNumber) {
    let valid = this.validate(containerNumber);
  ...
  this.validate = function (containerNumber) {
    ...
    if (!this.empty(containerNumber) && this.is_string(containerNumber)) {
      ...
      if (this.count(matches) !== 5) {
        ...
        let checkDigit = this.buildCheckDigit(matches);
  ...
  this.generate = function (ownerCode, productGroupCode, from, to) {
    ...
    let alphabetCode = this.strtoupper(ownerCode + productGroupCode);
    let containers_no = [];

    if (
      this.is_string(alphabetCode) &&
      this.strlen(ownerCode) === 3 &&
      this.strlen(productGroupCode) === 1
    ) {
      containers_no = [];
      let current_container_no = "";
      let current_container_check_digit = "";

      if (from >= 0 && to < 1000000 && to - from > 0) {
        for (var i = from; i <= to; i++) {
          current_container_no =
            alphabetCode + this.str_pad(i, 6, "0", STR_PAD_LEFT);
          current_container_check_digit =
            this.createCheckDigit(current_container_no);

          if (current_container_check_digit < 0) {
            this.errorMessages.push(
              "Error generating container number at number " + i
            );
            return containers_no;
          }

          containers_no[i] =
            current_container_no + current_container_check_digit;
        }
      } else {
        this.errorMessages.push(
  ...
  this.createCheckDigit = function (containerNumber) {
    let checkDigit = -1;
    if (!this.empty(containerNumber) && this.is_string(containerNumber)) {
      let matches = this.identify(containerNumber, true);
  ...
  this.buildCheckDigit = function (matches) {
    ...
    let numericalCode = this.array_merge(
      numericalOwnerCode,
      this.registrationDigit
    );
    let sumDigit = 0;
    ...
    let checkDigit = sumDigit - sumDigitDiff;
...
export default ContainerValidator;
